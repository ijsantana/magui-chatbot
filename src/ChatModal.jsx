...
const WELCOME_MESSAGE = '¡Hola! 👋 Bienvenido al chat. ¿En qué puedo ayudarte hoy?\n\nEscribe:\n**1** Ventas\n**2** Soporte\n**3** Información\n**4** Contacto'
  ...
  const handleSend = () => {
    ...
    if (response) {
      ...
      const optionsText = response.options
        ? '\n\n' +
          Object.entries(response.options)
            .map(([num, id]) => {
              const node = getNode(id)
              const label = id === 'root' ? 'Menú principal' : id
              return `**${num}** ${label}`
            })
            .join(' | ')
        : ''
      setMessages((prev) => [
        ...prev,
        { type: 'bot', text: response.message + optionsText },
      ])
    } else {
      setMessages((prev) => [
        ...prev,
        {
          type: 'bot',
          text: 'No reconozco esa opción. Por favor escribe 1, 2, 3 o 4 según lo que quieras hacer.',
        },
      ])
    }
  }
